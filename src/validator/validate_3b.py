...
"""
validate_3b.py
End-to-end validator for DSM-5 Task 3B (Symptom → Diagnosis vignette classification).

Input  : JSONL lines (disorder-level) with nested response.symptom_dx_items
Output : eval_3B_report.jsonl (one line per vignette with metrics + LLM judgment)
...
CFG = {
    ...
    "required_distribution": {"easy": 1, "moderate": 2, "hard": 2},
    "max_items": None,
    "seed": 1337
}
...
def structural_check(item: Dict[str, Any]) -> Tuple[bool, List[str]]:
    errs = []
    req = ["uuid","symptoms","options","answer","answer_index",
           "explanation","label_confidence","supporting_features",
           "evidence_span_indices","difficulty","source_disorder",
           "source_section","distractor_rationale"]
    ...
    if "distractor_rationale" in item and len(item.get("distractor_rationale", [])) != 3:
        errs.append("distractor_rationale_not_3")
...
def automated_metrics(item: Dict[str, Any]) -> Dict[str, Any]:
    sy = item.get("symptoms","")
    feats = item.get("supporting_features", [])

    sims = []
    for f in feats:
        sims.append(sim(sy, f))
    best_sim = max(sims) if sims else 0.0
    avg_sim = sum(sims)/len(sims) if sims else 0.0

    expl_len = len(item.get("explanation","").split())
    dist_rats = item.get("distractor_rationale", [])
    dist_short = sum(1 for d in dist_rats if len(d.split()) < CFG["distractor_min_words"])

    read_score = -1.0
    try:
        read_score = float(textstat.flesch_reading_ease(sy))
    except Exception:
        pass
...
def automated_rules(metrics: Dict[str, Any]) -> List[str]:
    ...
    if metrics["distractor_rationale_too_short"] > 0:
        warnings.append("short_distractor_rationales")
...
def main():
    ...
    ap.add_argument("--input", required=True, help="Path to 3B JSONL")
    ap.add_argument("--output", default="eval_3B_report.jsonl", help="Output JSONL")
    ...
    with open(args.input,"r",encoding="utf-8") as fin, open(args.output,"w",encoding="utf-8") as fout:
        for line in tqdm(fin,total=total_lines,desc="Validating disorders"):
            ...
            items = data.get("response",{}).get("symptom_dx_items", [])
            if not items:
                print(f"[WARN] {disorder} / {data.get('section')} -> no 3B items found")
                continue

            # --- check per-parent difficulty distribution ---
            parent_diffs = Counter([c.get("difficulty","unknown") for c in items])
            if any(parent_diffs[d] != CFG["required_distribution"][d] for d in CFG["required_distribution"]):
            ...
            for case in items:
                ...
                if struct_ok:
                    ...
                    if isinstance(case.get("label_confidence"), (float,int)):
                        confidence_stats[disorder].append(case["label_confidence"])
                ...
                out = {
                    ...
                    "answer_index": case.get("answer_index"),
                    "label_confidence": case.get("label_confidence")
                }
                fout.write(json.dumps(out,ensure_ascii=False)+"\n")

            print(f"[INFO] Processed {len(items)} 3B items for {disorder} / {data.get('section')}")
    ...
    summary = {
        ...
        "confidence_stats": {
            ...
            for dis, vals in confidence_stats.items() if vals
        },
    ...
    print("\nConfidence stats per disorder:")
    for dis, s in summary["confidence_stats"].items():
        print(f"  {dis:45s} mean={s['mean']:.3f} min={s['min']:.2f} max={s['max']:.2f} n={s['count']}")
    ...
    if summary["distribution_violations"]:
        print("\n⚠️ Parent difficulty distribution violations:")
